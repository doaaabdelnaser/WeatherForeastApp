...
import com.app.weatherforeastapp.ui.favourit.FavouritFragment
...
class MapsFragment : Fragment() ,GoogleMap.OnMapClickListener
{
    ...
    override fun onMapClick(point: LatLng) {
        ...
        if (arguments?.getInt("mapId")==1){
            val favouritesFragment= FavouritFragment()
            val bundle=Bundle()
            bundle.putDouble("lat",lat)
            bundle.putDouble("lon",lon)
            bundle.putInt("id",1)
            favouritesFragment.arguments=bundle
            findNavController().navigate(R.id.action_nav_map_to_nav_favourit,bundle)
        }else{
            ...
            findNavController().navigate(
                R.id.action_Fragment_to_nav_home
            )
        }
